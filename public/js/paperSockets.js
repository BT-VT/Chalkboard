...
export function paperSockets() {
  ...
  function textBackspace(lockOwner) {
    if (!initialPathsReceived || LOCKED != lockOwner) {
      return;
    }
  ...
  tool.onMouseDrag = function (event) {
    ...
    // paths = [ {pathName: "pathN", path: Path} ]
      ...
      let pathsItemArr = paths.filter(
        (pathsItem) => pathsItem.path == event.item
      );
  ...
  function drawTrackingRect(rectAttr) {
    ...
    curPath.remove();
    if (rectAttr.isEllipse) {
      curPath = new paper.Path.Ellipse(rectAttr);
    } else {
      curPath = new paper.Path.Rectangle(rectAttr);
    }
    curPath.onFrame = function (event) {
      this.dashOffset += attributes.dashOffset;
    };
  ...
  function deleteCurPath(owner) {
    ...
    console.log(
      "socket " + owner + " disconnected while drawing, releasing lock..."
    );
    curPath.remove();
    curPath = new paper.Path();
    unlockCanvas(owner);
  ...
  function setPathFunctions(pathsItem, scale) {
    ...
    path.onMouseDrag = function (event) {
      ...
      if (LOCKED != socket.id || drawingTools.grab != true) {
        return;
      }
      if (paper.Key.isDown("left")) {
        socket.emit(
          "requestPathRotate",
          -1 * attributes.rotation,
          pathInd,
          user
        );
  ...
  colorBtns.forEach((btn) => {
    btn.onclick = function () {
      ...
      if (window.selectedColor == "#c46f0f") {
        attributes.multicolor = true;
      } else {
        attributes.multicolor = false;
      }
